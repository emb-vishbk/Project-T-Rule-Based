"""Batch-generate Level-1 artifacts for all 20 Hz CAN sessions."""
...
ARTIFACT_SUBDIR = "level1_20hz"
SEGMENTS_FILENAME = "level1_segments.json"
...
def _resolve_sessions(args: argparse.Namespace) -> list[str]:
    from_file = _load_session_ids_file(args.session_ids_file)
    session_ids = from_file if from_file is not None else discover_session_ids(Path(args.data_root))
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-1 artifact generation for all HDD 20 Hz CAN sessions. "
            "This script executes the traceability Level-1 pipeline directly in-process."
        )
    )
    parser.add_argument("--data-root", default="data/can_data_20hz", help="Directory with <session_id>.npy files.")
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level1_20hz/<session_id>/...).")
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    from traceability.level1 import Level1Config, run_level1, write_level1_artifacts
    ...
    # 20 Hz profile derived from current 10 Hz profile by preserving temporal windows:
    # median_window: 5 -> 9 (preserve centered span), persistence: 3 -> 6 (preserve seconds).
    result = run_level1(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-1 20Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level1_20hz_segments: {existing_count}")
