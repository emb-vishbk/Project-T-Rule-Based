"""Batch-generate Level-1 artifacts for all 25 Hz CAN sessions."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-1 artifact generation for all HDD 25 Hz CAN sessions. "
    ...
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level1_25hz/<session_id>/...).")
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    # 25 Hz profile derived from current 10 Hz profile by preserving temporal behavior:
    # median_window: 5 -> 11 (exact centered-span preservation), persistence: 3 -> 8 (round 7.5 half-up).
    result = run_level1(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-1 25Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level1_25hz_segments: {existing_count}")
