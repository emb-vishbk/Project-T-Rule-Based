"""Batch-generate Level-1 artifacts for all 3 Hz CAN sessions."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-1 artifact generation for all HDD 3 Hz CAN sessions. "
    ...
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level1_3hz/<session_id>/...).")
    ...
    parser.add_argument(
        "--no-smoothing",
        action="store_true",
        help="Disable Level-1 speed median smoothing (uses median_window=1).",
    )
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    # 3 Hz profile: shorter smoothing/persistence than 10 Hz while keeping the same rule logic.
    ...
    result = run_level1(session_id=session_id, session_matrix=matrix, config=level1_config)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-1 3Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level1_3hz_segments: {existing_count}")
