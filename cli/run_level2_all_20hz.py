"""Batch-generate Level-2 artifacts for all 20 Hz CAN sessions."""
...
ARTIFACT_SUBDIR = "level2_20hz"
SEGMENTS_FILENAME = "level2_segments.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-2 artifact generation for all HDD 20 Hz CAN sessions. "
            "This script executes Level-1 and Level-2 pipelines directly in-process."
        )
    )
    parser.add_argument("--data-root", default="data/can_data_20hz", help="Directory with <session_id>.npy files.")
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level2_20hz/<session_id>/...).")
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    from traceability.level2 import Level2Config, run_level2, write_level2_artifacts
    ...
    # 20 Hz profile: preserve physical thresholds, scale smoothing/persistence in samples.
    level1_result = run_level1(
        session_id=session_id,
        session_matrix=matrix,
        config=Level1Config(fs_hz=20.0, median_window=9, v_stopped=0.5, v_moving=1.0, persistence_steps=6),
    )
    level2_result = run_level2(
        ...
        config=Level2Config(
            ...
            yaw_off=1.0,
            steer_on_deg=35.0,
            steer_speed_on_dps=80.0,
            assist_speed_min=2.0,
            straight_steer_max_deg=10.0,
            persistence_on_steps=16,
            persistence_off_steps=20,
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-2 20Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level2_20hz_segments: {existing_count}")
