"""Batch-generate Level-2 artifacts for all 25 Hz CAN sessions."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-2 artifact generation for all HDD 25 Hz CAN sessions. "
    ...
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level2_25hz/<session_id>/...).")
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    # 25 Hz profile: preserve physical thresholds, scale smoothing/persistence in samples.
    level1_result = run_level1(
        session_id=session_id,
        session_matrix=matrix,
        config=Level1Config(fs_hz=25.0, median_window=11, v_stopped=0.5, v_moving=1.0, persistence_steps=8),
    )
    level2_result = run_level2(
        ...
        config=Level2Config(
            ...
            persistence_on_steps=20,
            persistence_off_steps=25,
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-2 25Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level2_25hz_segments: {existing_count}")
