"""Batch-generate Level-2 artifacts for all 3 Hz CAN sessions."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-2 artifact generation for all HDD 3 Hz CAN sessions. "
    ...
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level2_3hz/<session_id>/...).")
    ...
    parser.add_argument(
        "--no-smoothing",
        action="store_true",
        help="Disable median smoothing in Level-1 and Level-2 preprocessing (uses window=1).",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-2 3Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level2_3hz_segments: {existing_count}")
