"""Batch-generate Level-3 artifacts for all 20 Hz CAN sessions."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-3 artifact generation for all HDD 20 Hz CAN sessions. "
    ...
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level3_20hz/<session_id>/...).")
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    # 20 Hz profile: preserve physical thresholds, scale median windows/persistence in samples.
    ...
    level3_result = run_level3(
        ...
        config=Level3Config(
            fs_hz=20.0,
            speed_median_window=13,
            accel_median_window=9,
            yaw_median_window=9,
            a_on=1.2,
            a_off=0.6,
            pedal_on=12.0,
            brake_on=150.0,
            pedal_cruise_max=8.0,
            brake_cruise_max=100.0,
            lon_persistence_steps=12,
            yaw_deadband=0.8,
            direction_persistence_steps=10,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-3 20Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level3_20hz_segments: {existing_count}")
