"""Batch-generate Level-3 artifacts for all 25 Hz CAN sessions."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Level-3 artifact generation for all HDD 25 Hz CAN sessions. "
    ...
    parser.add_argument("--artifacts-root", default="artifacts", help="Root output directory (writes artifacts/level3_25hz/<session_id>/...).")
...
def _process_session(session_id: str, args: argparse.Namespace) -> None:
    ...
    # 25 Hz profile: preserve physical thresholds, scale median windows/persistence in samples.
    ...
    level3_result = run_level3(
        ...
        config=Level3Config(
            fs_hz=25.0,
            speed_median_window=17,
            accel_median_window=11,
            yaw_median_window=11,
            a_on=1.2,
            a_off=0.6,
            pedal_on=12.0,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Level-3 25Hz batch run configuration:")
    ...
    print(f"- sessions_with_existing_level3_25hz_segments: {existing_count}")
