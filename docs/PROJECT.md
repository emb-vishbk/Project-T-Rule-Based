# CAN 25 Hz Dataset Context

This document describes the `data/can_data_25hz/` dataset that was prepared for downstream modeling and maneuver discovery.

## Honda Driving Dataset - Background
- Provenance: Each .npy file in data/can_data_25hz/ corresponds to one driving session from the Honda Research Institute Driving Dataset (HDD). HDD contains 137 sessions totaling ~104 hours of video, with an average session duration of ~45 minutes.
...
- In this workspace, we primarily use a CAN-only subset exported as one .npy per session, shaped (T,6) at 25 Hz with fixed column order and units, for unsupervised / rule-based maneuver discovery; video/labels are reserved for spot-checking and evaluation.

## What `can_data_25hz/` Contains

- Root folder: `data/can_data_25hz/`
- File format: one `.npy` file per driving session
- Total files: `137`
- Current layout is flat:
  - `data/can_data_25hz/<session_id>.npy`
  - Example: `can_data_25hz/201702271017.npy`
...
- shape: `(T, 6)`
- dtype: `float64`
- `T`: number of 25 Hz timesteps in that session.
...
## Dataset-Wide Stats (Current `can_data_25hz/`)
...
- Validation on `can_data_25hz` passed structure/rate/monotonicity checks.
